...
## КРИТИЧЕСКИЙ ИНВАРИАНТ: HTML и PDF — единый контент (PARITY)

**HTML-версия главы и PDF-версия ОБЯЗАНЫ содержать ОДИНАКОВЫЙ контент:**
те же абзацы, те же таблицы (с теми же колонками и строками), те же рисунки/
графики (тот же набор, по одному разу, в том же порядке).

**Источник истины — один: `backend/static/js/chapter_render.js`
(`window.renderChapter(blocks, cfg)`).** Именно он рисует HTML-главу и является
эталоном для PDF.

**Правило распространения изменений:**
> Если добавляется/удаляется/меняется ЛЮБОЙ контент (абзац, колонка таблицы,
> строка, рисунок, подпись) — изменение ОБЯЗАНО появиться И в HTML, И в PDF.
> Нельзя править одну версию, не приведя в соответствие вторую.

**Практика для PDF:**
1. Графики PDF — это скриншоты тех же Plotly-графиков, что в HTML
   (см. `backend/services/plotly_png_service.py`), а не отдельные matplotlib.
   Регенерируются при каждой сборке во временную папку, не хранятся.
2. Таблицы PDF должны иметь ТЕ ЖЕ колонки и строки, что HTML-таблицы
   (например, «Эффективность реагентов»: полная сводная таблица + детальная
   таблица по каждому вбросу — обе должны быть и в PDF).
3. Запрещено выводить сырой dict/JSON.
4. Никаких «лишних» графиков в одной версии, которых нет в другой
   (напр. дублирующийся сегментный график или график, отсутствующий в HTML).

**Проверка перед сдачей любой задачи по отчёту:** собрать HTML и PDF одной
скважины и сверить порисуночно/потаблично/поабзацно. Списки должны совпадать.

**НЕ НАРУШАТЬ**: расхождение контента HTML и PDF — это баг.
